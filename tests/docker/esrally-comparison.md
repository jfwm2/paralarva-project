es-rally docker performance comparison
======================================

Benchmarks to compare
---------------------
We compare the result of the benchmark done with our proxy to a baseline created using the same containers targeted
directly. Of course, the containers were deleted and re-created before each benchmark.
```
$ esrally list races
...
------------------------------------  ----------------  ----------  ------------------  -------------------  --------  --------------  ----------------  ---------------
c0195678-a6af-4241-b5f9-9aae71b15a7b  20210221T160943Z  percolator                      append-no-conflicts  external  localhost=920x  28147b6
e300907f-8cb2-402e-9728-5e4725b1a64c  20210221T143257Z  percolator                      append-no-conflicts  external  es1=30303       28147b6

```

Side-by-side comparison
-----------------------
Overall, there is no major decrease of performance when using our proxy (exact meaning of metrics used below can be 
found here: [summary report definitions]).

- the measured error rate is 0 for both benchmarks, with or without the proxy so the proxy does not induce any
  new unexpected behaviour.
- not surprisingly, overall throughput, latency (time between submission of a request and receiving the complete 
  response), and service time (time between start of request processing and receiving the complete response) results 
  are better without going through the proxy, but depending on the task performed results can vary.
- for some tasks (`percolator_with_content_president_bush`, `percolator_with_content_saddam_hussein`, and
  `percolator_with_content_ignore_me`), we have better results going through the proxy (not taking into account
  outliers, e.g., 100th percentiles, which is equivalent to the maximum of the dataset), whereas,
- for other ones (`index`, `percolator_no_score_with_content_google`, `percolator_with_highlighting`, and 
  `percolator_no_score_with_content_ignore_me`) direct access shows better performance, finally
- for the remaining tasks (`percolator_with_content_hurricane_katrina` and `percolator_with_content_google`), 
  throughput is almost the same with or without proxy (less than 1 ops/s difference), but latencies and service times
  vary:
  - The `percolator_with_content_hurricane_katrina` task shows slightly better latency (only for 99th and 100th
    percentiles) and service time when going through the proxy.
  - The `percolator_with_content_google` task lead to a better service time with direct access, but better latency
  going through the proxy.
    
It is interesting to denote that some metrics, like the `cumulative indexing time of primary shards` is up to 3 times
faster with the proxy.
This can be explained by the fact that the benchmarking tool seems to use only one endpoint for some actions, while
the proxy is balancing the load among the 3 elasticsearch containers.

Below is the output of the side-by-side comparison.
```
$ esrally compare --baseline c0195678-a6af-4241-b5f9-9aae71b15a7b --contender e300907f-8cb2-402e-9728-5e4725b1a64c
...
Comparing baseline
  Race ID: c0195678-a6af-4241-b5f9-9aae71b15a7b
  Race timestamp: 2021-02-21 16:09:43
  Challenge: append-no-conflicts
  Car: external
  User tags: localhost=920x

with contender
  Race ID: e300907f-8cb2-402e-9728-5e4725b1a64c
  Race timestamp: 2021-02-21 14:32:57
  Challenge: append-no-conflicts
  Car: external
  User tags: es1=30303
...
We use the commands below to generate report in csv ([esrally-compare.csv]) and markdown
([esrally-compare.md]) format; it is also available
in HTML format: [esrally-compare.html] with some colors to highlight results.

CSV file generation:
``` 
$ esrally compare --baseline c0195678-a6af-4241-b5f9-9aae71b15a7b --contender e300907f-8cb2-402e-9728-5e4725b1a64c --report-format csv --report-file tests/docker/esrally-compare.csv
```

Markdown file generation:
```
$ esrally compare --baseline c0195678-a6af-4241-b5f9-9aae71b15a7b --contender e300907f-8cb2-402e-9728-5e4725b1a64c --report-format markdown --report-file tests/docker/esrally-compare.md
```
